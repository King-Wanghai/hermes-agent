...
class AIAgent:
    ...
    def _create_openai_client(
        ...
        # Auto-detect custom endpoints that may be behind Cloudflare.
        # Cloudflare blocks requests with x-stainless-* headers and OpenAI/Python User-Agent.
        base_url = str(client_kwargs.get("base_url", "")).lower()
        is_known_provider = any(
            domain in base_url
            for domain in ["openrouter.ai", "api.openai.com", "api.anthropic.com"]
        )
        if base_url and not is_known_provider:
            import httpx

            class _CloudflareBypassTransport(httpx.HTTPTransport):
                def handle_request(self, request):
                    for h in list(request.headers.keys()):
                        if (
                            h.lower().startswith("x-stainless")
                            or h.lower() == "user-agent"
                        ):
                            del request.headers[h]
                    return super().handle_request(request)

            client = OpenAI(
                http_client=httpx.Client(transport=_CloudflareBypassTransport()),
                **client_kwargs,
            )
            logger.info(
                "OpenAI client created with Cloudflare bypass (%s, shared=%s) %s",
                reason,
                shared,
                self._client_log_context(),
            )
